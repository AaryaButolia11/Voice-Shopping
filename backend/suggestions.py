...
def build_suggestions(db: Session, limit: int = 6) -> list[dict]:
    ...
    def add(name: str, reason: str, kind: str, category: str | None = None):
    ...
    history = (
        ...
        .order_by(desc(PurchaseHistory.count), desc(PurchaseHistory.last_added))
        .limit(15)
        .all()
    )
    ...
    for produce in seasonal_items(month):
        add(produce, "In season right now", "seasonal", "produce")

    # 3. Substitutes for items already on the list.
    for item in db.query(Item).all():
        for sub in substitutes_for(item.name):
            add(sub, f"Alternative to {item.name}", "substitute")

    return suggestions[:limit]


def record_purchase(db: Session, name: str, category: str) -> None:
    ...
    if record:
        ...
        record = PurchaseHistory(name=key, category=category, count=1)
        db.add(record)
    db.commit()
