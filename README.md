...
**Speaklist** is a voice-based shopping assistant designed to make shopping-list

management as simple as speaking naturally.
...
The application combines **browser-based speech recognition, NLP, smart

recommendations, product search, authentication, and order generation** into

a single responsive application.
...
Voice commands can be recognized in multiple languages through the browser's
...
The application uses Groq-hosted Llama for structured intent parsing when an
...
This prevents the core shopping-list functionality from completely depending on
...
"You've added bread 3 times — running low?"
...
The application maintains a seasonal-product mapping and recommends products
...
Single-service deployment with FastAPI serving the frontend
...
.env is intentionally excluded from Git. Never commit API keys or other secrets.
...
https://voice-shopping-assistant-hqho.onrender.com/
...
Render connected to the repository

Render installs backend/requirements.txt

Uvicorn starts the FastAPI application

FastAPI serves the frontend as static files

Production database uses PostgreSQL

Environment secrets are configured through Render
...
The application includes basic production-oriented safeguards:
...
The browser already provides speech recognition capabilities without requiring

an additional speech-to-text infrastructure layer. This keeps the project

lightweight and suitable for an 8-hour implementation window.
...
LLMs are useful for interpreting flexible natural-language commands where

traditional keyword matching becomes brittle.
...
"Could you please put five oranges on my shopping list?"
...
"I think I need 5 oranges"



should both result in the same structured action.
...
An external LLM should not be a single point of failure for basic list

management. The fallback handles common command patterns deterministically.
...
The frontend is a static Vanilla JS application with no build step. FastAPI

can serve these files directly, reducing deployment complexity while keeping

the frontend and backend under one origin.
...
The product catalogue is demonstration/test data rather than a live retailer

catalogue.

Seasonal and substitute recommendations are based on predefined mappings.

LLM parsing depends on the availability and limits of the configured Groq
...
A concise 200-word approach write-up is included separately in:
...
https://github.com/AaryaButolia11/Voice-Shopping
...
1. Clone the repository
Bash
git clone [https://github.com/AaryaButolia11/Voice-Shopping.git](https://github.com/AaryaButolia11/Voice-Shopping.git)
cd Voice-Shopping
2. Create virtual environment
Bash
cd backend
...
give all in one readme.md code only
...
README
 MD 
Open
The requested README.md file for Speaklist — Voice Command Shopping Assistant has been generated and is available for download above.




Your VIT Bhopal University chats aren't used to improve our models. Gemini is AI and can make mistakes. Your privacy and GeminiOpens in a new window
